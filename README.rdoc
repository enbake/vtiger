...
There are a set of base commands using username and password  (eg http post, login etc)
And commands which are more specific to the actual usage such as addlead, retrieve_object

Options can be passed as hash  or preset using a Class variable
...
== SYNOPSIS:
   ...
   cmd = Vtiger::Mobile.new()
   login=cmd.login #returns true if login
   cmd.addlead  etc...
...
curl  http://crm.yoururl.com/modules/Mobile/api.php -X POST -d "_operation=login&username=admin&password=xyz"
